 ...
 const totalRevenue = useMemo(
  ...
  >
      ...
      <table width="100%" cellPadding="8" style={{ borderCollapse: "collapse" }}>
        ...
        <tbody>
            {orders.map((o) => {
              const isGas = !!o.gas_cylinder; // 👈 THIS LINE FIXES EVERYTHING

              return (
              <tr key={o.id} style={{ borderBottom: "1px solid #eee" }}>
               <td>{new Date(o.business_date).toLocaleDateString()}</td>

               <td>{isGas ? o.gas_cylinder : ""}</td>

               <td>{!isGas ? "1" : ""}</td>

               <td>{isGas ? o.quantity : ""}</td>

               <td>{isGas ? `R ${o.unit_price}` : ""}</td>

              <td>
                {isGas
                ? `R ${(o.quantity * o.unit_price).toLocaleString()}`
                : ""}
              </td>
